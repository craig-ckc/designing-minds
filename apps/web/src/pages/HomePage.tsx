...
function FeaturedResourcesSection({ snapshot, loadError }: { snapshot: CmsSnapshot | null; loadError?: string | null }) {
  const featured = snapshot ? getFeatured(snapshot, 6) : []
    ...
    <section className="section bg-surface-alt">
      <Container>
        ...
        {snapshot ? (
          ...
          <SectionNotice
            title={loadError ? 'Featured resources are unavailable' : 'Loading featured resources...'}
            body={loadError ?? 'Products will appear here as soon as the catalogue request finishes.'}
          />
        )}
      </Container>
    </section>
...
function HomeTestimonialsSection({ snapshot, loadError }: { snapshot: CmsSnapshot | null; loadError?: string | null }) {
  if (snapshot) return <TestimonialsSection testimonials={snapshot.testimonials.filter((t) => t.published && t.featured)} />
    ...
    <section className="section bg-surface-alt">
      <Container>
        <SectionNotice
          ...
          body={loadError ?? 'Parent feedback will appear here when the content request finishes.'}
        />
      </Container>
    </section>
  )
...
export function HomePage({ snapshot, loadError }: { snapshot: CmsSnapshot | null; loadError?: string | null }) {
  return (
    <>
      ...
      <FeaturedResourcesSection snapshot={snapshot} loadError={loadError} />
      <WhyParentsSection />
      <HomeTestimonialsSection snapshot={snapshot} loadError={loadError} />
